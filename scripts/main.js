;(function (window) {
  ...
  function setDimensions () {
    CANVASWIDTH = Math.max(300, Math.floor(window.innerWidth * 0.45))
    CANVASHEIGHT = Math.max(150, Math.floor(window.innerHeight * 0.38))
  ...
  function event () {
    ...
    canvas.addEventListener('click', function () { cycle() }, false)
    canvas.addEventListener('touchstart', function () { cycle() }, false)
  }
  ...
  if(!isChrome){ if(window.$) $('#iframeAudio').remove() }
  init()
})(window)
